...
path = "/home/Documents/python-tuto/velo.csv"
check_path = "/home/Documents/python-tuto/"
...
#df.writeStream \
	#.outputMode("append") \
	#.option("es.resource", "rani23") \
	#.option("es.port", "9200") \
	#.option("es.nodes", "localhost") \
	#.start()


query = df.writeStream.format('consol').option("es.port", "9200").option("es.nodes" , "localhost").option("es.resource", "ranii23").outputMode("append").start()
#sparkSession = SparkSession.builder().config(ConfigurationOptions.ES_NODES, "127.0.0.1").config(ConfigurationOptions.ES_PORT, "9200").appName("streaming").getOrCreate()
